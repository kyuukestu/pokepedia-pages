...
// Filter for Kanto and sort by date (ascending)
const kantoEvents = computed(() => {
  return eventList
    .filter((event) => event.extendedProps.region.includes('Kanto'))
  ...
  <v-container fluid class="pa-0">
    ...
    <v-container max-width="900" class="py-12">
      <div class="events-timeline-wrapper">
        <v-row v-if="kantoEvents.length > 0">
          <v-col cols="12" v-for="(event, index) in kantoEvents" :key="index">
            <v-card
              flat
              border
              rounded="xl"
        ...
        <v-row v-else justify="center" class="py-12">
          <v-col cols="12" class="text-center">
            ...
            <div class="text-h6 text-grey mt-4">No Kanto events found for this period.</div>
          </v-col>
        </v-row>
      </div>
    </v-container>
